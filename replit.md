...
- `index.html` — Public ticketera. Mercado Pago buttons trigger `POST /api/mp/preference` with the per-sale token as `external_reference`, then redirect to MP's `init_point`. Transfer fallback kept. Generates client-side token `ISM-NNNN-AA` per sale. Live FOMO banner. Tracks RRPP click on `?ref=` once per session.
...
- `POST /api/mp/preference` — body `{ token, tipo, cantidad, total, nombre?, dni? }`. Calls Mercado Pago `/checkout/preferences` with `external_reference = token`, `back_urls.success = /success.html?token=...`, `notification_url = /api/mp/webhook`, `auto_return = "approved"`. Returns `{ init_point, sandbox_init_point, preference_id }`.
- `POST /api/mp/webhook` — receives MP notification. Always responds 200 immediately. Fetches `/v1/payments/{id}` from MP using `MP_ACCESS_TOKEN`, reads `external_reference` (the EGRESANTOS token), and if `status === "approved"` patches the matching `ventas` doc to `estado: "confirmada"` via Firestore REST API (no Admin SDK / no service account; uses public Web API key + permissive rules).
- `MP_ACCESS_TOKEN` secret is required.
- Webhook URL to configure in MP panel: `https://<your-domain>/api/mp/webhook`.

### Tech
Tailwind CDN, Firebase v10 (front-end), qrcode.js (CDN), Express 5 (api-server), Firestore REST (server-side writes). Inter italic 900, magenta #c2007e, dark #0a0a0a glassmorphism.
